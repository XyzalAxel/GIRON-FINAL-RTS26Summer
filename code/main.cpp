 ...
 *
 * ============================================================
 * Theme: Ride Safety Interrupt System
 * ============================================================
 */
...
static void btn_task_sem(void *arg)
{
    for (;;) {
        if (xSemaphoreTake(btn_sem, portMAX_DELAY) == pdTRUE) {
            ...
            ESP_LOGI(TAG, "[sem] E-STOP | press #%lu | latency=%lld us | max=%llu us",
...
static void btn_task_notif(void *arg)
{
    for (;;) {
        ...
        ESP_LOGI(TAG, "[notif] E-STOP | press #%lu | latency=%lld us | max=%llu us | notif count=%lu",
                 (unsigned long)presses_observed,
                 (long long)lat,
...
#define B_SAMP 32                       /* power of two for the index mask */
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "==== App 3 [Ride Safety Interrupt System] starting — ISR + bottom-half ====");
    ...
    xTaskCreatePinnedToCore(btn_task_sem,  "btn_sem",   6144, NULL, 12, NULL, APP_CPU_NUM);
    xTaskCreatePinnedToCore(btn_task_notif,"btn_notif", 6144, NULL, 12,
